...
class Interfuser(nn.Module):
    def __init__(
        ...
        self.traffic_seg_head_type = traffic_det_head_type
        self.num_features = (
            self.embed_dim
        ...
        if self.traffic_seg_head_type == "seg":
            self.traffic_seg_head = PyramidParsingNet()
            '''
            self.traffic_seg_head = nn.Sequential(
                ...
                nn.ReLU(),
                nn.ConvTranspose2d(32, 28, kernel_size=4, stride=2, padding=1),    
                nn.ReLU(),
    ...
    def forward(self, x):
        ...
        n, b, c = memory.shape
        h = w = int(math.sqrt(n - 5))
        spatial_features = memory[:h*w].permute(1, 2, 0).view(bs, 256, 12, 12)
        ...
        traffic_feature_reshaped = traffic_feature.view(bs, -1, 256)  # Reshape to (B, H*W, C)
        seg_input = traffic_feature_reshaped.permute(0, 2, 1).view(bs, 256, 20, 20)

        segmentation = self.traffic_seg_head(seg_input)
        segmentation = segmentation.view(-1,28,224,224)
        return traffic, waypoints, is_junction, traffic_light_state, stop_sign, traffic_feature, segmentation, memory


@register_model
def bevdriver_encoder_train(**kwargs):
    model = Interfuser(
        enc_depth=6,
        dec_depth=6,
        embed_dim=256,
...
class PyramidParsingNet(nn.Module):
    def __init__(self):
        super(PyramidParsingNet, self).__init__()
        
        # These are the original working convolutional layers
        ...
        # Let's add a simpler pyramid pooling module at 256 channels
        self.ppm = PyramidPoolingModule(
            in_channels=256,
            pool_sizes=[1, 2, 3, 6],
            ppm_out_channels=16  # Keeping this small to avoid overwhelming the features
        )
        
        # Projection to match channels after concatenation
        # 256 + (16 * 4) = 320 channels after PPM
    ...
    def forward(self, x):
        ...
        # Add skip connection
        x = x + x  # Original features preserved
        
        # Apply the proven convolutional path
        x = self.conv_path(x)
...
class PyramidPoolingModule(nn.Module):
    def __init__(self, in_channels, pool_sizes, ppm_out_channels):
        ...
        for pool_size in pool_sizes:
            path = nn.Sequential(
                ...
                nn.Conv2d(in_channels, ppm_out_channels, 1),
