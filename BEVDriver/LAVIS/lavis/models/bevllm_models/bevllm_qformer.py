...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x, view):
        pos_enc = self.classify_views(x, view)
        return x + pos_enc


# new shared model class
class BEVLLMQformer(BEVLLMBase):
    ...
    def __init__(
        ...
        cwd = Path.cwd()
        bev_config =f"{cwd}/mmdetection3d/projects/BEVFusion/configs/bevfusion_lidar-cam_voxel0075_second_secfpn_8xb4-cyclic-20e_nus-3d.py"
        bev_checkpoint = f"{cwd}/mmdetection3d/projects/BEVFusion/bevfusion_lidar-cam_voxel0075_second_secfpn_8xb4-cyclic-20e_nus-3d-5239b1af.pth" #TODO
        self.bev_fusion = init_model(bev_config, bev_checkpoint, rank)
        for name, param in self.bev_fusion.named_parameters():
            param.requires_grad = False

        self.Qformer, self.query_tokens = self.init_Qformer(
            num_query_token, 512, cross_attention_freq
        )

        scale = 0.06
        #self.view_position_embeddings = PositionalEncoding(scale, 512, 6, feature_space_shape)
    ...
    def forward(self, samples):
        ...
        bev_embeds = self.bev_fusion.test_step(bev)[0] #TODO
        ...
        print(f'AAAA: {bev_embeds.shape}')
        # adapt shape to [bs, patch, feats]
        ...
        # normalize the output of linear layer and project to shared embedding space
    ...
    def extract_features(self, samples, mode="multimodal"):
        ...
        if mode == "bev":
            ...
            bev_embeds = torch.flatten(bev_embeds, start_dim=2)
            # pos encoding
            #bev_embeds = self.view_position_embeddings(bev_embeds, self.view) 
